...
for section_name, questions in faqs.items():
    ...
    <div class="faq-category" style="margin-bottom: 60px;">
        <h2 style="font-size: 24px; font-weight: 600; color: #fff; margin-bottom: 24px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 12px;">{section_name}</h2>
        <div class="faq-list" style="display: flex; flex-direction: column; gap: 16px;">
    """
    
    for q, a in questions:
        # Glassmorphism accordion using <details>
        item_html = f"""
            <details class="faq-item" style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; overflow: hidden; transition: background 0.3s ease;">
                <summary style="padding: 24px; font-size: 18px; font-weight: 500; color: #fff; cursor: pointer; list-style: none; display: flex; justify-content: space-between; align-items: center; outline: none;">
                    {q}
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="faq-icon" style="transition: transform 0.3s ease; opacity: 0.6;"><polyline points="6 9 12 15 18 9"></polyline></svg>
                </summary>
                <div style="padding: 0 24px 24px 24px; font-size: 16px; color: #94A3B8; line-height: 1.6;">
                    {a}
                </div>
            </details>
        """
...
<section class="section is-faq" style="padding-bottom: 120px;">
  <div class="container-medium" style="max-width: 800px; margin: 0 auto; padding: 0 5%;">
      {all_faqs}
  </div>
  <style>
      .faq-item:hover {{
...
# Find hero heading and update text
hero_title = soup.find(string=lambda text: text and 'Frequently Asked Questions' in text)
if hero_title:
    pass # Already correct from previous script

hero_desc = soup.find('p', class_='paragraph-style-x')
if hero_desc:
    hero_desc.string = "Everything you need to know about Nexmart, Agentic Commerce, shopping, orders, payments, shipping, and using our AI-powered commerce platform."

# Find placeholder section and replace
...
print("Successfully injected FAQ content.")
